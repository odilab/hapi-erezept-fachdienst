...
public class AcceptOperationProvider implements IResourceProvider {
    ...
    public Bundle acceptTaskOperation(
        ...
        try {
            ...
            String providedAccessCode = accessCode != null ? accessCode.getValue() : null;
            validateAccessCode(providedAccessCode, theRequestDetails, task);
    ...
    private void validateTaskStatus(Task task, String telematikId) {
        ...
        if (status.equals(Task.TaskStatus.INPROGRESS)) {
            if (task.hasOwner() && task.getOwner().getDisplay() != null && 
                ...
                throw new ResourceVersionConflictException("Task has invalid status in-progress. Task is processed by requesting institution");
            } else {
                throw new ResourceVersionConflictException("Task has invalid status in-progress");
            }
        } else if (status.equals(Task.TaskStatus.DRAFT)) {
            throw new ResourceVersionConflictException("Task has invalid status draft");
        } else if (status.equals(Task.TaskStatus.COMPLETED)) {
            throw new ResourceVersionConflictException("Task has invalid status completed");
        } else if (!status.equals(Task.TaskStatus.READY)) {
            throw new ResourceVersionConflictException("Task must be in status ready");
        }
    }
    ...
    private Binary loadPrescriptionBinary(Task task) {
        ...
        if (healthCarePrescriptionUuid == null) {
            LOGGER.warn("Task hat keine HealthCarePrescriptionUuid Extension");
            return null;
        }
        
        // Lade Binary mit dieser ID
        IFhirResourceDao<Binary> binaryDao = daoRegistry.getResourceDao(Binary.class);
        try {
            Binary binary = binaryDao.read(new IdType("Binary", healthCarePrescriptionUuid), null);
            LOGGER.debug("Binary mit ID {} erfolgreich geladen", healthCarePrescriptionUuid);
            return binary;
        } catch (ResourceNotFoundException e) {
            LOGGER.error("Binary mit ID {} nicht gefunden", healthCarePrescriptionUuid);
            return null;
        }
    }
    ...
    private void addConsentIfExists(Bundle bundle, Task task) {
        // Extrahiere KVNR aus Task
        String kvnr = extractKvnr(task);
        if (kvnr == null) {
            return;
        }
        
        // Suche Consent für diese KVNR
        IFhirResourceDao<Consent> consentDao = daoRegistry.getResourceDao(Consent.class);
        try {
            // Suche nach Consent mit patient.identifier = kvnr und category = "CHARGCONS"
            // Dies würde normalerweise über SearchParameter erfolgen
            // Hier vereinfachte Implementation - in Produktion würde man SearchParameterMap verwenden
            LOGGER.debug("Suche nach Consent für KVNR: {}", kvnr);
            
            // TODO: Implementierung der Consent-Suche mit SearchParameterMap
            // SearchParameterMap params = new SearchParameterMap();
            // params.add(Consent.SP_PATIENT, new ReferenceParam("Patient/" + kvnr));
            // params.add(Consent.SP_CATEGORY, new TokenParam("CHARGCONS"));
            // IBundleProvider results = consentDao.search(params);
            
        } catch (Exception e) {
            LOGGER.warn("Fehler beim Suchen nach Consent: {}", e.getMessage());
        }
    }
