...
public class AbortOperationProvider implements IResourceProvider {
     ...
     * 
     * Alternativ können die Parameter auch als FHIR Parameters Resource übergeben werden.
     * 
     * @param theId Die Task-ID (Prescription ID)
     * @param theAccessCode AccessCode Parameter (optional)
     * @param theSecret Secret Parameter (optional)
    ...
    public Task abortTaskOperation(
            ...
            @OperationParam(name = "ac") StringType theAccessCode,
            @OperationParam(name = "secret") StringType theSecret,
            RequestDetails theRequestDetails) {
        
        LOGGER.info("$abort Operation aufgerufen für Task ID: {}", theId.getIdPart());

        try {
            // Parameter extrahieren - unterstütze sowohl Query-Parameter als auch FHIR Operation Parameter
            String accessCode = null;
            String secret = null;
            
            // 1. Versuche Query-Parameter zu lesen
            String[] accessCodeParams = theRequestDetails.getParameters().get("ac");
            if (accessCodeParams != null && accessCodeParams.length > 0) {
                accessCode = accessCodeParams[0];
            }
            
            String[] secretParams = theRequestDetails.getParameters().get("secret");
            if (secretParams != null && secretParams.length > 0) {
                secret = secretParams[0];
            }
            
            // 2. Falls keine Query-Parameter vorhanden, verwende Operation Parameter
            if (accessCode == null && theAccessCode != null && theAccessCode.hasValue()) {
                accessCode = theAccessCode.getValue();
            }
            if (secret == null && theSecret != null && theSecret.hasValue()) {
                secret = theSecret.getValue();
            }
            
            // 3. Extrahiere und validiere Access Token
            ...
            LOGGER.info("Abort-Operation: Profession={}, ProfessionOid={}", accessToken.getProfession(), professionOid);
            ...
            // 7. Flowtype-spezifische Prüfung für Versicherte (A_22102_01)
            if ("oid_versicherter".equals(professionOid)) {
                checkFlowtypeRestrictions(task);
            }

            // 8. Rollenbasierte Validierung
            ...
            // 9. Setze Task Status auf cancelled (A_19121)
            // HINWEIS: In HAPI 8.0.0 werden Tasks mit Status CANCELLED möglicherweise als gelöscht behandelt
            // und sind danach nicht mehr abrufbar. Dies ist möglicherweise gewünschtes Verhalten.
            task.setStatus(Task.TaskStatus.CANCELLED);
            task.getMeta().setLastUpdated(new Date());

            // 10. Lösche personenbezogene Daten aus dem Task selbst (A_19027-06)
            // Dies entfernt Referenzen und sensitive Daten aus dem Task
            abortTaskService.clearPersonalDataFromTask(task);

            // 11. WICHTIG: Speichere den Task ZUERST, bevor referenzierte Ressourcen gelöscht werden
            // Sonst könnten Referenzvalidierungen fehlschlagen
            ...
            // DEBUG: Versuche den Task direkt nach dem Update wieder zu lesen
            try {
                Task testRead = taskDao.read(theId, theRequestDetails);
                LOGGER.info("DEBUG: Task nach Update lesbar - Status: {}", testRead.getStatus());
            } catch (Exception e) {
                LOGGER.error("DEBUG: Task nach Update NICHT lesbar: {}", e.getMessage());
            }
            
            // 12. NACH dem Task-Update: Lösche verknüpfte Ressourcen
            // Dies erfolgt NACH dem Task-Update, damit keine Referenzvalidierungsfehler auftreten
            // Gemäß A_19027-06 müssen alle verknüpften personenbezogenen Daten gelöscht werden
            abortTaskService.deleteCommunicationsForTask(theId.getIdPart());
            abortTaskService.deleteHealthCarePrescriptionBinary(task);
            abortTaskService.deleteReceiptBundle(task);
            abortTaskService.deleteMedicationDispenses(theId.getIdPart());
            ...
            // 14. Return den aktualisierten Task
            // Wichtig: Der Task muss zurückgegeben werden, damit er nach dem Update noch abrufbar ist
    ...
    private String performRoleBasedValidation(
        ...
        // Andere Nutzer (Versicherte, Ärzte)
            ...
            // Versicherte
            if ("oid_versicherter".equals(professionOid)) {
                String kvnrFromToken = accessToken.getKvnr().orElse(null);
                String kvnrFromTask = extractKvnr(task);
                
                LOGGER.info("Abort für Versicherten - KVNR aus Token: {}, KVNR aus Task: {}", kvnrFromToken, kvnrFromTask);
                
                if (kvnrFromTask == null) {
                    throw new UnprocessableEntityException("Task has no KV number");
                }
                
                if (kvnrFromToken == null) {
                    throw new UnprocessableEntityException("Token has no KVNR");
                }
                
                // A_20546_03: Eigenes Rezept (KVNR stimmt überein)
                if (kvnrFromToken.equals(kvnrFromTask)) {
                    LOGGER.info("KVNR stimmt überein - Patient löscht eigenes Rezept");
     ...
     */
    private void validateAccessCode(String providedAccessCode, Task task, RequestDetails requestDetails) {
    ...
    private String extractKvnr(Task task) {
        if (task.hasFor()) {
            Reference forRef = task.getFor();
            // KVNR kann entweder als Identifier oder als direkte Reference gespeichert sein
            if (forRef.hasIdentifier()) {
                return forRef.getIdentifier().getValue();
            } else if (forRef.hasReference()) {
                // Format: "Patient/KVNR"
                String ref = forRef.getReference();
