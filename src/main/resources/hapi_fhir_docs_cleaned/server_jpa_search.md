---
source: https://hapifhir.io/hapi-fhir/docs/server_jpa/search.html
crawled: 2025-08-01T14:05:33.600931
---

# Server Jpa Search

#  5.6.1JPA Server Search
[ ](https://hapifhir.io/hapi-fhir/docs/server_jpa/search.html#jpa-server-search)
...
#  5.6.2Limitations
[ ](https://hapifhir.io/hapi-fhir/docs/server_jpa/search.html#limitations)
...
5.6.2.0.1Chains within _has Chains within _has are not currently supported for performance reasons. For example, this search is not currently supported ```
https://localhost:8000/Practitioner?_has:PractitionerRole:practitioner:service.type=CHIRO

```
Copy 5.6.2.0.2Location.position "near" Searching on Location.Position using `near` currently uses a box search, not a radius search. A box search is a square centred on the provided coordinates with the shortest distance to the edge of the square equal to the distance provided; this means the box search will match more locations than a radius search in the corners. Currently, distance is assumed to be in km and any provided units are ignored. Distance must be between 0.0km and 10,000km. 5.6.2.0.3_filter The special `_filter` is only partially implemented. 5.6.2.0.4_pid The JPA server implements a non-standard special `_pid` which matches/sorts on the raw internal database id. This sort is useful for imposing tie-breaking sort order in an efficient way. Note that this is an internal feature that may change or be removed in the future. Use with caution.
#  5.6.3Uplifted Refchains and Chaining Performance
[ ](https://hapifhir.io/hapi-fhir/docs/server_jpa/search.html#uplifted-refchains)
...
For example, consider the search: [http://example.org/Encounter?subject.name=Simpson](http://example.org/Encounter?subject.name=Simpson)
...
##  5.6.3.1Drawbacks[](https://hapifhir.io/hapi-fhir/docs/server_jpa/search.html#drawbacks)
...
##  5.6.3.2Defining Uplifted Refchains[](https://hapifhir.io/hapi-fhir/docs/server_jpa/search.html#defining-uplifted-refchains)
...
The extension has the following URL: [https://smilecdr.com/fhir/ns/StructureDefinition/searchparameter-uplift-refchain](https://smilecdr.com/fhir/ns/StructureDefinition/searchparameter-uplift-refchain)
...
Copy
#  5.6.4Document and Message Search Parameters
[ ](https://hapifhir.io/hapi-fhir/docs/server_jpa/search.html#document-and-message-search-parameters)
...
These parameters are only intended to be used as a part of a chained search expression, since it would not be meaningful to use them otherwise. For example, the following query could be used in order to use the _composition_ Search Parameter to locate FHIR Documents stored on the server that are an [International Patient Summary](https://hapifhir.io/hapi-fhir/docs/server_jpa/ips.html). In other words, this searches for Bundle resources where the first resource in the Bundle is a Composition, and that Composition has a `Composition.type` using LOINC code `60591-5`. [https://hapi.fhir.org/baseR4/Bundle?composition.type=http://loinc.org%7C60591-5](https://hapi.fhir.org/baseR4/Bundle?composition.type=http://loinc.org%7C60591-5)
...
Copy
In order to use these Search Parameters, you must enable [Uplifted Refchains](https://hapifhir.io/hapi-fhir/docs/server_jpa/search.html#uplifted-refchains) on your server, and modify the SearchParameter resource in order to add Uplifteed Refchain definitions for any chained searches you wish to support.
...
Copy
#  5.6.5Chained Sorting
[ ](https://hapifhir.io/hapi-fhir/docs/server_jpa/search.html#chained-sorting)
...
The following qualified expression adds a type qualifier and will work: [http://example.org/Encounter?_sort=Patient:subject.family](http://example.org/Encounter?_sort=Patient:subject.family)
##  5.6.5.1Chained Sort Performance[](https://hapifhir.io/hapi-fhir/docs/server_jpa/search.html#chained-sort-performance)
...
In order to improve sorting performance when chained sorts are needed, an [Uplifted Refchain](https://hapifhir.io/hapi-fhir/docs/server_jpa/search.html#uplifted-refchains) can be defined on the SearchParameter. This index will be used for the sorting expression and can improve performance.
#  5.6.6_include and _revinclude order
[ ](https://hapifhir.io/hapi-fhir/docs/server_jpa/search.html#-include-and-revinclude-order)
...
#  5.6.7Custom Search Parameters
[ ](https://hapifhir.io/hapi-fhir/docs/server_jpa/search.html#custom-search-parameters)
