---
source: https://hapifhir.io/hapi-fhir/docs/server_jpa_partitioning/partitioning.html
crawled: 2025-08-01T14:09:21.162728
---

# Server Jpa Partitioning Partitioning

#  7.0.1Partitioning and Multitenancy
[ ](https://hapifhir.io/hapi-fhir/docs/server_jpa_partitioning/partitioning.html#partitioning-and-multitenancy)
...
#  7.0.2Architecture
[ ](https://hapifhir.io/hapi-fhir/docs/server_jpa_partitioning/partitioning.html#architecture)
##  7.0.2.1Conceptual Architecture[](https://hapifhir.io/hapi-fhir/docs/server_jpa_partitioning/partitioning.html#conceptual-architecture)
...
Mappings between the **Partition Name** and the **Partition ID** are maintained using the [Partition Management Operations](https://hapifhir.io/hapi-fhir/docs/server_jpa_partitioning/partitioning_management_operations.html).
##  7.0.2.2Logical Architecture[](https://hapifhir.io/hapi-fhir/docs/server_jpa_partitioning/partitioning.html#logical-architecture)
At the database level, partitioning involves the use of two dedicated columns to many tables within the HAPI FHIR JPA [database schema](https://hapifhir.io/hapi-fhir/docs/server_jpa/schema.html):
  * **PARTITION_ID** – This is an integer indicating the specific partition that a given resource is placed in. This column can also be _NULL_ , meaning that the given resource is in the **Default Partition**.
...
When partitioning is used, these two columns will be populated with the same value for a given resource on all resource-specific tables (this includes [HFJ_RESOURCE](https://hapifhir.io/hapi-fhir/docs/server_jpa/schema.html#HFJ_RESOURCE) and all tables that have a foreign key relationship to it including [HFJ_RES_VER](https://hapifhir.io/hapi-fhir/docs/server_jpa/schema.html#HFJ_RES_VER), [HFJ_RESLINK](https://hapifhir.io/hapi-fhir/docs/server_jpa/schema.html#HFJ_RES_LINK), [HFJ_SPIDX_*](https://hapifhir.io/hapi-fhir/docs/server_jpa/schema.html#search-indexes), etc.)
When a new resource is **created** , an [interceptor hook](https://hapifhir.io/hapi-fhir/docs/server_jpa_partitioning/partitioning.html#partition-interceptors) is invoked to request the partition ID and date to be assigned to the resource.
When a resource is **updated** , the partition ID and date from the previous version will be used.
When a **read operation** is being performed (e.g. a read, search, history, etc.), a separate [interceptor hook](https://hapifhir.io/hapi-fhir/docs/server_jpa_partitioning/partitioning.html#partition-interceptors) is invoked in order to determine whether the operation should target a specific partition. The outcome of this hook determines how the partitioning manifests itself to the end user:
...
[ ](https://hapifhir.io/hapi-fhir/docs/server_jpa_partitioning/partitioning.html#partitioning-and-resource-ids)
...
This fact can have security implications:
  ...
  * In a server using the default configuration of SEQUENTIAL_NUMERIC [Server ID Strategy](https://hapifhir.io/hapi-fhir/apidocs/hapi-fhir-storage/ca/uhn/fhir/jpa/api/config/JpaStorageSettings.html#setResourceServerIdStrategy\(ca.uhn.fhir.jpa.api.config.JpaStorageSettings.IdStrategyEnum\)) a client may be able to infer the IDs of resources in other partitions based on the ID they were assigned.
...
[ ](https://hapifhir.io/hapi-fhir/docs/server_jpa_partitioning/partitioning.html#partition-interceptors)
...
##  7.0.4.1Identify Partition for Create (Required)[](https://hapifhir.io/hapi-fhir/docs/server_jpa_partitioning/partitioning.html#identify-partition-for-create-required)
A hook against the [`Pointcut.STORAGE_PARTITION_IDENTIFY_CREATE`](https://hapifhir.io/hapi-fhir/apidocs/hapi-fhir-base/ca/uhn/fhir/interceptor/api/Pointcut.html#STORAGE_PARTITION_IDENTIFY_CREATE) pointcut must be registered, and this hook method will be invoked every time a resource is created in order to determine the partition the resource is assigned to.
The criteria for determining the partition will depend on your use case. For example:
  * If you are implementing multi-tenancy the partition might be determined by using the [Request Tenant ID](https://hapifhir.io/hapi-fhir/docs/server_plain/multitenancy.html). It could also be determined by looking at request headers, or the authorized user/session context, etc.
...
##  7.0.4.2Identify Partition for Read (Optional)[](https://hapifhir.io/hapi-fhir/docs/server_jpa_partitioning/partitioning.html#identify-partition-for-read-optional)
A hook against the [`Pointcut.STORAGE_PARTITION_IDENTIFY_READ`](https://hapifhir.io/hapi-fhir/apidocs/hapi-fhir-base/ca/uhn/fhir/interceptor/api/Pointcut.html#STORAGE_PARTITION_IDENTIFY_READ) pointcut must be registered, and this hook method will be invoked every time a resource is read in order to determine the partition to read the resource from.
As of HAPI FHIR 5.3.0, the _Identify Partition for Read_ hook method may return multiple partition names or IDs. If more than one partition is identified, the server will search in all identified partitions.
##  7.0.4.3Non-Partitionable Resources[](https://hapifhir.io/hapi-fhir/docs/server_jpa_partitioning/partitioning.html#non-partitionable-resources)
Some resource types can not be placed in any partition other than the DEFAULT partition. When a resource of one of these types is being created, the _STORAGE_PARTITION_IDENTIFY_CREATE_ pointcut is invoked, but the hook method must return [defaultPartition()](https://hapifhir.io/hapi-fhir/apidocs/hapi-fhir-base/ca/uhn/fhir/interceptor/model/RequestPartitionId.html#defaultPartition\(\)). A partition date may optionally be included.
...
##  7.0.4.4Examples[](https://hapifhir.io/hapi-fhir/docs/server_jpa_partitioning/partitioning.html#examples)
See [Partition Interceptor Examples](https://hapifhir.io/hapi-fhir/docs/server_jpa_partitioning/partition_interceptor_examples.html) for various samples of how partitioning interceptors can be set up.
#  7.0.5Complete Example: Using Request Tenants
[ ](https://hapifhir.io/hapi-fhir/docs/server_jpa_partitioning/partitioning.html#complete-example-using-request-tenants)
In order to achieve a multitenant configuration, the following configuration steps must be taken:
  ...
  * A [Tenant Identification Strategy](https://hapifhir.io/hapi-fhir/docs/server_plain/multitenancy.html) must be enabled on the RestfulServer.
  * A [RequestTenantPartitionInterceptor](https://hapifhir.io/hapi-fhir/docs/interceptors/built_in_server_interceptors.html#request-tenant-partition-interceptor) instance must be registered as an interceptor.
...
#  7.0.6Limitations
[ ](https://hapifhir.io/hapi-fhir/docs/server_jpa_partitioning/partitioning.html#limitations)
...
7.0 Partitioning and Multitenancy 
