...
class CustomValidatorTest  {
    ...
    void testValidateValidErgInvoice() {
        ...
        Patient ergPatient = createTestErgPatient();
        Practitioner ergPractitioner = createTestErgPractitioner();
        Organization ergInstitution = createTestErgInstitution();
        // Minimales ChargeItem für die Referenz in der Invoice
        ChargeItem chargeItem = createMinimalChargeItem(ergPatient); 
        ...
        Invoice ergInvoice = createValidErgInvoice(ergPatient, ergPractitioner, ergInstitution, chargeItem);

        // 4. ERG-DocumentReference erstellen, die auf die gespeicherten Ressourcen und die Invoice verweist
        DocumentReference ergDocRef = createValidErgDocumentReference(ergPatient, ergPractitioner, ergInstitution, ergInvoice, anhangDocRefId);
	...
	void testValidateValidErgDocumentReference() {
		...
		Practitioner ergPractitioner = createTestErgPractitioner();
		Organization ergInstitution = createTestErgInstitution();
		ChargeItem chargeItem = createMinimalChargeItem(ergPatient); // Für die Invoice benötigt
	...
	void testValidateErgDocumentReference_MissingSignatureConstraint() {
		...
		// 7. Überprüfe die Fehlermeldung (wird nur erreicht, wenn assertThrows fehlschlägt)
        // Dieser Teil sollte im Erfolgsfall (also wenn der Test wie erwartet fehlschlägt) nicht erreicht werden.
        // Wenn er erreicht wird, hat assertThrows keine Exception gefangen.
        /*
		String exceptionMessage = "Exception was expected but not thrown.";
        OperationOutcome outcome = null; // Kann nicht abgerufen werden, wenn keine Exception geworfen wird.
        try {
            // Versuche, die Exception zu bekommen, die assertThrows gefangen hätte
            client.create().resource(ergDocRef).withAdditionalHeader("Authorization", "Bearer " + getValidAccessToken("HBA_ARZT")).execute();
        } catch (UnprocessableEntityException e) {
            exceptionMessage = e.getMessage();
            outcome = (OperationOutcome) e.getOperationOutcome();
        } catch (Exception e) {
            exceptionMessage = "An unexpected exception occurred: " + e.getMessage();
        }
		String diagnostics = outcome != null && outcome.hasIssue() ? outcome.getIssueFirstRep().getDiagnostics() : "";

		assertTrue(
			exceptionMessage.contains("SignaturVerpflichtendRechnung") || diagnostics.contains("SignaturVerpflichtendRechnung") || // Constraint Key
			exceptionMessage.contains("Signature muss vorhanden sein") || diagnostics.contains("Signature muss vorhanden sein") || // Constraint Human Text
            exceptionMessage.contains("erg-docref-signature") || diagnostics.contains("erg-docref-signature"), // Extension URL
			"Fehlermeldung sollte die fehlende Signatur oder die Constraint 'SignaturVerpflichtendRechnung' erwähnen. Got: " + exceptionMessage + " | Diagnostics: " + diagnostics
		);
		logger.info("Test erfolgreich fehlgeschlagen mit erwarteter Validierungsfehlermeldung bezüglich fehlender Signatur.");
        */
	}
	 ...
	 */
	private DocumentReference createValidErgDocumentReference(Patient patient, Practitioner practitioner, Organization institution, Invoice invoice, String anhangDocRefId) {
		...
		// Extension: behandlungsart - Wurde entfernt, da hier nicht erlaubt.

		return docRef;
	}


	private Invoice createValidErgInvoice(Patient patient, Practitioner practitioner, Organization institution, ChargeItem chargeItem) {
        ...
        // Money price = chargeItem.hasPriceOverride() ? chargeItem.getPriceOverride() : new Money().setCurrency("EUR").setValue(10.72); // Bereits oben geholt
     ...
     */
    private ChargeItem createMinimalChargeItem(Patient patient) {
    ...
    private Patient createTestErgPatient() {
    ...
    private Practitioner createTestErgPractitioner() {
        ...
        // USt-ID: Setze sowohl text (gemäß Pattern) als auch coding (gemäß Fehlermeldung)
        // >> Erneut auskommentiert für Test <<
        // practitioner.addIdentifier()
        //     .setType(new CodeableConcept()
        //         .setText("UmsatzsteuerId") // Gemäß Pattern
        //         .addCoding(new Coding("http://terminology.hl7.org/CodeSystem/v2-0203", "TAX", null))) // Gemäß Fehlermeldung (TAX ist in VS IdentifierType)
        //     .setSystem("urn:oid:1.3.6.1.4.1.12345.1.1.1") // Beispiel OID
    ...
    private Organization createTestErgInstitution() {
