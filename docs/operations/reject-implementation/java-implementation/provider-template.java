...
public class RejectOperationProvider implements IResourceProvider {
     ...
     * @param secret Das Secret als Query-Parameter (erforderlich)
     * @param theRequestDetails Request-Details mit Authorization-Header
     * @return Kein Body (HTTP 204 No Content)
     */
    @Operation(name = "$reject", idempotent = false, type = Task.class)
    public void rejectTaskOperation(
        ...
        try {
            ...
            // 6. Validiere Secret (A_19171-03 + A_20703)
            validateSecret(secret, task);

            // 7. Führe Reject-Operation aus
            ...
            // 9. Kein Response-Body (HTTP 204 No Content) - A_19514
            // Spring/HAPI handhabt dies automatisch wenn void zurückgegeben wird
    ...
    private void validateSecret(StringType secretParam, Task task) {
        ...
        if (secretParam == null || !secretParam.hasValue() || secretParam.getValue().isEmpty()) {
        ...
        String providedSecret = secretParam.getValue();
